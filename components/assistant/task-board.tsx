...
import { CheckCircle, Circle, Clock, AlertCircle, AlertTriangle } from 'lucide-react'
      ...
      <div className="flex-1 space-y-3 overflow-y-auto">
        {tasks.map((task: any) => {
          ...
          return (
            <div 
              key={task.id} 
              className={`bg-white p-3 rounded-lg shadow-sm border group relative overflow-hidden transition-all hover:shadow-md
                ${isUrgent ? 'border-red-200 animate-pulse-slow' : 'border-slate-200'}`}
            >
              {isUrgent && (
