          ...
          {attentionItems.map((item) => (
            ...
            >
              <div
                ...
                style={{ backgroundColor: item.type === 'danger' ? 'var(--danger)' : 'var(--warning)' }}
              />
          ...
          {activeTrades.map((trade) => (
            <div key={trade.id} data-testid={`trade-${trade.id}`} className="flex items-center justify-between px-3 py-2.5 rounded-[var(--radius-md)] inset-material">
              <div className="flex items-center gap-2">
                <div
                  className="w-1.5 h-1.5 rounded-full shrink-0"
                  style={{ backgroundColor: trade.status === 'on-site' ? 'var(--success)' : trade.status === 'scheduled' ? 'var(--warning)' : 'var(--text-muted)' }}
                />
                <span className="text-xs font-medium text-[var(--text-primary)]">{trade.name}</span>
              </div>
              <span
                className="text-[10px] font-medium capitalize"
                style={{ color: trade.status === 'on-site' ? 'var(--success)' : trade.status === 'scheduled' ? 'var(--warning)' : 'var(--text-muted)' }}
              >
